...
void main() {
  ...
  final filteredImage = x_edge_Filter(image);
  File('x_filtered_resim.jpg').writeAsBytesSync(encodeJpg(filteredImage));
}

Image x_edge_Filter(Image image) {
  final x_edge_Kernel = [1, -2, 1];
    ...
    for (var x = 1; x < image.width - 1; x++) {
      ...
      for (var ky = -1; ky <= 1; ky++) {
        for (var kx = -1; kx <= 1; kx++) {
          ...
          final kernelValue = x_edge_Kernel[kx + 1];
